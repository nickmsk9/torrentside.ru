<?php
// game.php — Upload Game on TorrentSide (PHP 8.1-ready)
...
// Заголовки
stdhead("Загрузить игру на TorrentSide");
...
if ($descrtype === null) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo "<tr><td class='lol' align='center'><a href=\"game.php?type={$cid}\">{$cname}</a></td></tr>";
    }
    echo "</table>";
    stdfoot();
    exit;
...
<div style="text-align:center">
  <p><span style="color:green;font-weight:bold;">
    После загрузки торрента вам нужно будет скачать его и начать сидировать из папки с оригинальными файлами.
  </span></p>

  <form id="upload" name="upload" enctype="multipart/form-data" action="takeuploadgame.php" method="post" accept-charset="UTF-8" autocomplete="off">
    ...
    <table border="1" cellspacing="0" cellpadding="5" style="margin:0 auto;min-width:800px">
      ...
      tr("Оригинальное название", '<input type="text" name="origname" size="60" maxlength="200" placeholder="например — Doom 3">', 1);

      // Обложка
      $imgHelp = '<input type="url" name="image0" size="60" maxlength="500" placeholder="https://..." pattern="https?://.+" />'
        . '<br><b>Укажите URL картинки</b>';
      tr($tracker_lang['images'] ?? 'Обложка', $imgHelp, 1);

      // Год/Разработчик/Издатель/Жанр/Язык
      tr("Год выхода", '<input type="number" name="year" min="1980" max="' . date('Y') . '" size="6" placeholder="' . date('Y') . '">', 1);
      tr("Разработчик", '<input type="text" name="director" size="40" maxlength="120" placeholder="id Software">', 1);
      tr("Издатель", '<input type="text" name="publisher" size="40" maxlength="120" placeholder="Activision/Bethesda">', 1);
      tr("Жанр", '<input type="text" name="janr" size="40" maxlength="120" placeholder="Action, Shooter" required>', 1);
