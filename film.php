...
if ($type === null) {
    ...
    $cats = tracker_cache_get($MC_KEY_CATS, $cacheHit);
    if (!$cacheHit || !is_array($cats)) {
        ...
        tracker_cache_set($MC_KEY_CATS, $cats, 300);
    }
...
$catName = tracker_cache_get($MC_KEY_CAT, $cacheHit);
if (!$cacheHit || !is_string($catName)) {
    $res = sql_query("SELECT name FROM categories WHERE id = " . (int)$descrtype . " LIMIT 1");
    $row = mysqli_fetch_row($res);
    $catName = (string)($row[0] ?? '');
    tracker_cache_set($MC_KEY_CAT, $catName, 300);
}
